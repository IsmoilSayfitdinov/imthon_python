...
class Menejers(Base):
    ...
    @staticmethod
    def select_inner_join():
        query = f"""SELECT menejers.firstname, menejers.lastname, menejers.username, menejers.password, menejers.email, menejers.phone, address.name FROM menejers INNER JOIN address ON customer.address_id = address.address_id;  """
        return Dbconnect.connect(query, "select")
...
class Product_status(Base):
    def __init__(self, name):
        self.name = name

    def insert(self):
        query = f"""INSERT INTO product_status(name) VALUES ('{self.name}')"""
        return Dbconnect.connect(query, "insert")


class Dostavka(Base):
    def __init__(self, products_s, product):
        self.products_s = products_s
        self.product = product

    def insert(self):
        query = f"INSERT INTO dostavka(product_status, product) VALUES ('{self.products_s}', '{self.product}')"
        return Dbconnect.connect(query, "insert")

    @staticmethod
    def select_inner_join():
        query = " SELECT dostavka.dostavka_id,  product_status.name, product.name  FROM dostavka INNER JOIN product_status ON dostavka.product_status = product_status.product_s_id INNER JOIN product ON dostavka.product = product.product_id;"
        return Dbconnect.connect(query, "select")
